        ...
        <div className="flex bg-slate-50 min-h-screen">
            ...
            <main className="flex-1 p-8">
                <header className="mb-10">
                    <h2 className="text-3xl font-black text-slate-800">My Learning</h2>
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {enrolledCourses.map(course => (
                            <div key={course._id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex items-center gap-6 group hover:shadow-md transition">
                                <div className="h-24 w-24 bg-indigo-100 rounded-xl flex items-center justify-center text-indigo-600 group-hover:bg-indigo-600 group-hover:text-white transition">
                                    <BookOpen size={32} />
                                </div>
                                <div className="flex-1">
                                    <h3 className="text-xl font-bold text-slate-800">{course.title}</h3>
                                    <p className="text-slate-500 text-sm mb-3">{course.category}</p>
                                    <div className="flex items-center gap-4">
                                        <button 
                                            onClick={() => navigate(`/course-view/${course._id}`)}
                                            className="flex items-center gap-2 bg-slate-900 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-slate-700 transition"
                                        >
